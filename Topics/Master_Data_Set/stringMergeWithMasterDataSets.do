	...
	*  DIME Wiki article on file paths for explanation.
	...
	*Translitteration discrepancies from languages with different script
	replace first_name = "Mahmood" 		if first_name == "Mahmod" 		& last_name == "Al-Meghribi" & village == "Mountain Bend"

	*Applying the case convention used in the master data set
	...
	/*Merge the monitoring data set with the master data set. This step will 
		require a lot of preparation and it is common that one could have to go 
		back and make numerous additions above before the merge works without 
		errors, but after this work is done here, it is done once and for all and
		your data sets will merge and append effortlessly throughout your project. 
		Here some important details on the command below:
		-Only use 1:1 matching in this case. Stata will then give you an 
		 ...
		 to add more identifying varaibles or solve ambiguous cases manually 
		 before you can proceed.
		-assert(using match). This option tests that all observation after the 
		 ...
		 order to know what observations cause an error in assert(match using) 
